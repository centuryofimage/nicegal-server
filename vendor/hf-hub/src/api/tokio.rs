...
pub trait Progress {
    ...
    /// Whether the caller has requested that the active transfer stop.
...
pub enum ApiError {
    /// The caller cancelled the active download.
...
impl ApiRepo {
    ...
    async fn download_tempfile<P: Progress + Clone + Send + Sync + 'static>(
        ...
        if cancelled || progressbar.is_cancelled() {
            return Err(ApiError::Cancelled);
        }
    ...
    pub async fn download_with_progress<P: Progress + Clone + Send + Sync + 'static>(
        ...
        let tmp_filename = match self
            .download_tempfile(&url, metadata.size, tmp_path.clone(), progress)
            .await
        {
            Err(ApiError::Cancelled) => {
