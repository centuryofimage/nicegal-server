...
impl ThumbnailDb {
    ...
    pub fn store_video_samples_with_output(
        ...
        for sample in samples {
            ...
            let posters =
                poster::raster_buckets_with_output(&sample.raster, &super::SIZE_BUCKETS, output)?;
            encoded.push((sample.timestamp_ms, posters));
        }
        ...
        )?;
        {
            ...
            for (timestamp_ms, posters) in &encoded {
        ...
        for (&bucket, poster) in super::SIZE_BUCKETS.iter().zip(&encoded[0].1) {
            tx.execute(
