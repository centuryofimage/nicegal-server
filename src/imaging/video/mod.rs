#![forbid(unsafe_code)]
//! FFmpeg is confined to this synchronous reader; callers use the bounded video service.
use std::sync::{
    Arc,
...
impl VideoReader {
    pub(crate) fn open(path: &Utf8Path, cancelled: Arc<AtomicBool>) -> Result<Self> {
