...
actual sampled presentation timestamp. `embedding_id` is the `vec0` row identifier; there is no
separate sample catalog or generation table. A video's vectors and state are replaced together in
one transaction. Search scores current samples, picks each asset's lowest-distance sample before
